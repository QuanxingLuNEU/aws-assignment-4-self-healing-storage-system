...
def lambda_handler(event, context):
    ...
    # 1. First Upload: Small baseline file (approx 19 bytes)
    s3.put_object(
        Bucket=BUCKET_NAME,
        Key="assignment1.txt",
        Body="Empty Assignment 1".encode('utf-8')
    )
    print("Step 1: Created assignment1.txt (19 bytes).")
    ...
    # CloudWatch Metrics aggregate data every 60 seconds (1-minute period).
    # We wait ~75 seconds to allow the Alarm to transition to 'ALARM' state 
    # and trigger the Cleaner Lambda.
    print("Waiting 75s for CloudWatch Alarm to fire and Cleaner to remove the largest file...")
    time.sleep(75)

    # 4. Third Upload: Trigger a second cleanup cycle
    # At this point, assignment2.txt should have been deleted by the Cleaner.
    # Current bucket expected state: assignment1 (19 bytes) + assignment3 (2 bytes) = 21 bytes.
    s3.put_object(
        Bucket=BUCKET_NAME,
        Key="assignment3.txt",
        Body="333".encode('utf-8')
    )
